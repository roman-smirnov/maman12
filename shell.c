...
int runCommand(struct job newJob, struct jobSet *jobList,
    ...
    if (!strcmp(newJob.progs[0].argv[0], "exit")) {
        ...
        if (newJob.progs[0].argv[1][0] != '%') {
            /* first character in 2nd arg must be a '%' */
            printf("syntax error\n");
            return 1;
        }

        /* get the job number  in base 10, or 0 if error */
        jobNum = (int) strtol(++newJob.progs[0].argv[1], NULL, 10);
        if (jobNum == 0) {
            /* something is wrong with the input digits */
            printf("syntax error\n");
            return 1;
        }
        ...
        for (job = jobList->head; job; job = job->next) {
            if (job->jobId == jobNum) {
                ...
                if (newJob.progs[0].argv[0][0] == 'f') {
                    /* if it's a "fg" command - move the job to foreground */
                    if (tcsetpgrp(0, job->pgrp)) {
                        /* failed */
                        perror("tcsetpgrp");
                        return 1;
                    } else {
                        /* success - update joblist */
                        jobList->fg = job;
                    }
                } else {
                    /* do the job to background */
                }

                /* mark all child programs as not-stopped */
                for (i = 0; i < job->numProgs; ++i) {
                    job->progs[i].isStopped = 0;
                }
                /* number of stopped child programs as 0 */
                job->stoppedProgs = 0;
        ...
        /* a job with the given number wasn't found */
        printf("no such job\n");
        return 1;

        /* end fg and bg commadn handling */
    }
    ...
    for (
        ...
        if (!(newJob.progs[i].
            ...
            setpgid(newJob
                            .progs[i].pid, newJob.progs[0].pid);

            execvp(newJob
                           .progs[i].argv[0], newJob.progs[i].argv);
            fprintf(stderr,
                    "exec() of %s failed: %s\n",
